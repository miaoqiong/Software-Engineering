...
import static Servlets.ProcessTopkStudent.givenappCategoryAllstudent;
import static Servlets.ProcessTopkStudent.rankResutl;
import static Servlets.ProcessTopkStudent.topkstudent;
...
@WebServlet(name = "topkmostusedstudents", urlPatterns = {"/json/top-k-most-used-students"})
public class topkmostusedstudents extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (commonValidation && allValid) {
            ...
            for (Map.Entry<Integer, TopkUsageResult> entry : finalresult.entrySet()) {
                ...
                //a list of studen name with macaddress within an arraylist
                ...
                for (int i = 0; i < names.size(); i++) {
                    String newlist = names.get(i).trim();
                    String[] macName = newlist.split(",");
                    sortedList.put(macName[1], macName[0]);
                }

                for (Map.Entry<String, String> entry2 : sortedList.entrySet()) {
                    subArr.addProperty("rank", key);
                    subArr.addProperty("name", entry2.getKey());
                    subArr.addProperty("mac-address", entry2.getValue());
